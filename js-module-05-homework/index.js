...
const getAllNames = arr =>
  arr.reduce((acc, element) => acc.concat(element.name), []);
...
const getUsersByGender = (arr, gender) => {
  const usersByGender = arr.filter(element => element.gender === gender);
  const usersByGenderName = usersByGender.reduce(
    (acc, element) => acc.concat(element.name),
    []
  );
  return usersByGenderName;
};
...
const getInactiveUsers = arr => {
  const inactiveUsers = arr.filter(element => element.isActive === false);
...
const getUserByEmail = (arr, email) => {
  const userByEmail = arr.find(element => element.email === email);
  return userByEmail;
};
...
const getTotalBalance = arr => {
  const totalBalance = arr.reduce((acc, element) => acc + element.balance, 0);
  return totalBalance;
};
...
const getUsersByFriend = (arr, name) => {
  const usersByFriend = arr.filter(element => element.friends.includes(name));
  const usersByFriendName = usersByFriend.reduce(
    (acc, element) => acc.concat(element.name),
    []
  );
